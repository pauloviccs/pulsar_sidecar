...
import { NowPlayingScreen, FriendsPanel } from "../components/pulsar-screens";
...
type Overlay = "queue" | "chat" | "connect" | "playlist" | "now" | "friends" | null;
...
function PulsarApp() {
  ...
  return (
    <div className="relative flex h-dvh min-h-[680px] w-full overflow-hidden bg-background text-foreground">
      ...
      <main className="relative z-10 flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-background/62 px-4 backdrop-blur-xl sm:px-7">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <Button variant="icon" onClick={() => setOverlay("friends")} aria-label="Amigos"><Users size={17}/></Button>
      ...
      <CommunityPanel onChat={() => setOverlay("friends")}/>
      ...
      {overlay === "now" && <NowPlayingScreen playing={playing} onPlay={() => setPlaying(!playing)} close={() => setOverlay(null)} onQueue={() => setOverlay("queue")}/>}
      {overlay === "friends" && <FriendsPanel close={() => setOverlay(null)}/>}
      {overlay && overlay !== "now" && overlay !== "friends" && <OverlayView type={overlay} close={() => setOverlay(null)} playing={playing} onPlay={() => setPlaying(!playing)}/>} 
    </div>
  );
}
