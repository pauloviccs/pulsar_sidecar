...
function TrackRows({onPlay}:{onPlay:()=>void}) { return <div className="glass-panel overflow-hidden rounded-xl">{tracks.concat(tracks.slice(0,2)).map((t,i)=><button key={`${t[0]}${i}`} onClick={onPlay} className="grid w-full grid-cols-[42px_1fr_auto] items-center gap-4 border-b border-border p-3 text-left last:border-0 hover:bg-accent"><img src={playlists[i%4].image} alt="" className="size-10 rounded-md object-cover"/><span><b className="block text-sm">{t[0]}</b><small className="text-muted-foreground">{t[1]}</small></span><MoreHorizontal size={17} className="text-muted-foreground"/></button>)}</div> }
